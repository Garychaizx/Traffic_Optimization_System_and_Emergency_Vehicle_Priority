...
def detect_siren_from_audio(audio_file_path):
    ...
    if features is not None:
        ...
        if prediction[0][0] > 0.5:
            print("🚨 Siren detected by CNN model!")
            return True
    return False
...
class Simulation:
    ...
    def _get_state(self):
        ...
        for car_id in car_list:
            ...
            distance_to_light = 750 - lane_pos
            ...
            # Mark emergency vehicle if close to intersection
            # This system is using RFID control that can interrupt the normal sequence of traffic light to change the
            # path that emergency vehicle is travelling on into green after a few second
            if traci.vehicle.getTypeID(car_id) == "emergency_veh" and distance_to_light < 100:
                emergency_close = True
                emergency_lane = traci.vehicle.getRoadID(car_id)

        # Example simulated file path (replace with dynamic sounds or simulated sound input)
        audio_file_path = "dynamic_sounds/ambulance.wav"

        if emergency_close and detect_siren_from_audio(audio_file_path):
            print(f"🚨 Siren detected on approach: {emergency_lane}")
            directions = {"N2TL": 80, "S2TL": 81, "E2TL": 82, "W2TL": 83}
            for direction_prefix, state_idx in directions.items():
                if emergency_lane.startswith(direction_prefix):
                    state[state_idx] = 1
